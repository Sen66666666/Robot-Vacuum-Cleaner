...
def generate_launch_description():
    ...
    sdf_path = os.path.join(get_package_share_directory('coursework2'), 'worlds', 'tb3.sdf')

    if 'IGN_GAZEBO_RESOURCE_PATH' in os.environ:
        gz_world_path =  os.environ['IGN_GAZEBO_RESOURCE_PATH'] + os.pathsep + os.path.join(get_package_share_directory('coursework2'), "worlds")
    else:
        gz_world_path =  os.path.join(get_package_share_directory('coursework2'), "worlds")
    ...
    node_rviz = Node(
        ...
        executable='rviz2',
        name='rviz2',
        arguments=['-d' + os.path.join(get_package_share_directory('coursework2'), 'rviz', 'view_robot.rviz')]
    )
